...
# Parameters
POP_LIST=("African" "African-small" "Asian" "Asian-small" "Indian" "Indian-small" "European" "European-small")
NCAUSAL_LIST=(100)
SPEC_LIST=(0 50 100)
SNR_LIST=(5 10)
FOLD_LIST=$(seq 1 10)
...
# Slurm parameters
PART=candes,hns,normal,stat  # Partition names
MEMO=2G                     # Memory required (2G)
TIME=00:20:00               # Time required (20m)
    ...
    for NCAUSAL in "${NCAUSAL_LIST[@]}"; do    
      for SNR in "${SNR_LIST[@]}"; do    
        for SPEC in "${SPEC_LIST[@]}"; do    
          ...
          if [[ ! -f $OUT_FILE ]]; then
            # Script to be run
            SCRIPT="analysis_others.sh $POP $NCAUSAL $SPEC $SNR $FOLD"
            # Define job name
