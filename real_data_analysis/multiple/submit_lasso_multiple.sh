...
#POP_LIST=("Asian")
PHENO_LIST=("bmi" "cvd" "diabetes" "sbp")
#PHENO_LIST=("height")
...
for RESOLUTION in {1..6}; do
  for POP in "${POP_LIST[@]}"; do
    for PHENO in "${PHENO_LIST[@]}"; do
      OUT_FILE=$OUT_DIR"/lasso_multiple_res"$RESOLUTION"_"$POPULATION"_"$PHENO  
      if [[ ! -f $OUT_FILE ]]; then
          # Script to be run
          SCRIPT="lasso_multiple.sh $POP $RESOLUTION $PHENO"
            
          # Define job name
          JOBN="lasso_multiple"$POP"_"$RESOLUTION"_"$PHENO
          OUTF=$LOGS"/"$JOBN".out"
          ERRF=$LOGS"/"$JOBN".err"
