...
for(resolution in resolution_list){
  file_dir <- sprintf("%s/transfer_gwas/gwas/results/%s_res%d_fdr%.1f_offset%d_%s_multi.txt", 
                      oak_dir, pheno_name, resolution, alpha, offset, relatedness)
  res <- read_delim(file_dir, delim = " ", col_types = cols()) 
  all_res <- rbind(all_res, res)
}

method_list <- c("Vanilla Knockoffs", "Adaptive Weighted Knockoffs", "Weighted Lasso", "Weighted Lasso (Multiple Prior)")
new_method_list <- c("Vanilla", "Transfer - adaptive (gam)", "Transfer - weighted-lasso", "Transfer - multi-weighted-lasso")
resolution_list <- 1:6
