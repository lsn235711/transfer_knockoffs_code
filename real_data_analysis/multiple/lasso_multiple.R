...
if(sum_stat){
  ## Read the info on cvd
  cvd.file <- sprintf("%s/popstruct/analysis/importance/lasso_%s_british_related_res%d.txt",
                        oak, "cvd", res)

  bmi.file <- sprintf("%s/popstruct/analysis/importance/lasso_%s_british_related_res%d.txt",
                        oak, "bmi", res)
  
  diabetes.file <- sprintf("%s/popstruct/analysis/importance/lasso_%s_british_related_res%d.txt",
                        oak, "diabetes", res)
  
  sbp.file <- sprintf("%s/popstruct/analysis/importance/lasso_%s_british_related_res%d.txt",
                        oak, "sbp", res)
  
  if(res != 0){
    cvd.prior <- read_delim(cvd.file, delim=" ", col_types=cols()) %>%
    ...
    bmi.prior <- read_delim(bmi.file, delim=" ", col_types=cols()) %>%
      mutate(Knockoff=endsWith(SNP, ".k"), SNP=str_replace(SNP, ".k", "")) %>%
      full_join(Groups, by=c("CHR", "SNP")) %>%
      mutate(Z = ifelse(is.na(Z), 0, Z), Beta = ifelse(is.na(Beta), 0, Beta)) %>%
      group_by(CHR, Group, SNP) %>%
    ...
    diabetes.prior <- read_delim(diabetes.file, delim=" ", col_types=cols()) %>%
      mutate(Knockoff=endsWith(SNP, ".k"), SNP=str_replace(SNP, ".k", "")) %>%
      full_join(Groups, by=c("CHR", "SNP")) %>%
      mutate(Z = ifelse(is.na(Z), 0, Z), Beta = ifelse(is.na(Beta), 0, Beta)) %>%
      group_by(CHR, Group, SNP) %>%
    ...
    sbp.prior <- read_delim(sbp.file, delim=" ", col_types=cols()) %>%
      mutate(Knockoff=endsWith(SNP, ".k"), SNP=str_replace(SNP, ".k", "")) %>%
      full_join(Groups, by=c("CHR", "SNP")) %>%
      mutate(Z = ifelse(is.na(Z), 0, Z), Beta = ifelse(is.na(Beta), 0, Beta)) %>%
      group_by(CHR, Group, SNP) %>%
    ...
    Beta.prior <- cvd.prior %>% full_join(bmi.prior, by = c("CHR", "SNP", "Group")) %>%
      full_join(diabetes.prior, by = c("CHR", "SNP", "Group")) %>%
      full_join(sbp.prior, by = c("CHR", "SNP", "Group")) %>%
      ...
      mutate(Z = (cvd + bmi + diabetes + sbp) / 4)
...
out.file <- sprintf("%s/transfer_gwas/gwas/stats/lasso_multiple_res%d_%s_%s.txt",
